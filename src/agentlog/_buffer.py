...
def get_debug_context(
    ...
    *,
    ...
    Auto-includes errors first, git state, token usage, then fills remaining
    budget with relevant events. Session-scoped when active unless an explicit
    session_id is provided.

    Args:
        max_tokens: Approximate token budget (1 token ~ 4 chars).
        token_budget: v2 alias for max_tokens.
        incident_id: Optional incident filter.
        session_id: Optional session filter. Defaults to current active session.
    ...
    active_session = session_id if session_id is not None else _core._session_id

    return assemble_debug_context(
        entries,
        max_tokens=budget,
